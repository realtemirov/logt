...
func main() {
	...
	w := l.NewWriter("myService.myFunction", false)
	defer w.Close()

	ctx := l.SetContext(context.Background(), "data", 123456, &User{
		Firstname: "",
		Lastname:  "",
		Email:     "",
		Password:  "",
	...
	w.Succes("log from myService")
	w.FromContext(ctx).Succes("log from myService")
}
